...
static int encrp_add = -1, encrp_shift = -1, buffer_mode = -1;

void encrp_hhyeo(char *, size_t);
void decrp_hhyeo(char *, size_t);

void encrp_hhyeo(char *buffer, size_t size)
{
    int i;
    char *temp; 
    ...
    if(size < encrp_shift)
    {
        ...
        for (i = 0; i < size; i++) {
            buffer[i] = temp[(i + encrp_shift) % size];
        }
    }
    else
    {
        temp = (char *)malloc(encrp_shift * sizeof(char));
        memcpy(temp, buffer + size - encrp_shift, encrp_shift);
        memmove(buffer + encrp_shift, buffer, size - encrp_shift);
        memcpy(buffer, temp, encrp_shift);
    }

    free(temp);
}

void decrp_hhyeo(char *buffer, size_t size)
{
    int i;
    char *temp; 
    ...
    if(size < encrp_shift)
    {
        ...
        memcpy(temp, buffer, size);
        for (i = 0; i < size; i++) {
            buffer[(i - encrp_shift) % size] = temp[i];
        }
    }
    else
    {
        temp = (char *)malloc(encrp_shift * sizeof(char));
        memcpy(temp, buffer, encrp_shift);
        memmove(buffer, buffer + encrp_shift, size - encrp_shift);
        memcpy(buffer + size - encrp_shift, temp, encrp_shift);
    }
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    log_msg("\nbb_read(path=\"%s\", before_decrption=\"%s\")\n", path, buf);

    if (retstat < 0)
    	retstat = bb_error("bb_read read");
    else if(retstat > 0)
        decrp_hhyeo(buf, retstat - 1);
    
    log_msg("\nbb_read(path=\"%s\", retstat=0x%08x, after_decrption=\"%s\")\n", path, retstat, buf);
	     ...
	     struct fuse_file_info *fi)
{
    ...
    log_msg("\nbb_write(path=\"%s\", before_encryption=\"%s\")\n", path, buf);

    encrp_buf = (char *)malloc(size * sizeof(char));
    memcpy(encrp_buf, buf, size);
    encrp_hhyeo(encrp_buf, size - 1);
    // no need to get fpath on this one, since I work from fi->fh not the path
    log_fi(fi);

    retstat = pwrite(fi->fh, encrp_buf, size, offset);
    if (retstat < 0)
        retstat = bb_error("bb_write pwrite");

    log_msg("\nbb_write(path=\"%s\", retstat=0x%08x, after_encryption=\"%s\")\n", path, retstat, encrp_buf);
...
int main(int argc, char *argv[])
{
    ...
    // setting user configuration (encryntion, buffer cache):w
    FILE *fp = fopen("ee516.conf", "r");
    fscanf(fp, "%d\t", &encrp_add);
    fscanf(fp, "%d\n", &encrp_shift);
    fscanf(fp, "%d\n", &buffer_mode);
    printf("encrp_add: %d, encrp_shift: %d, buffer_mode: %d\n", encrp_add, encrp_shift, buffer_mode);
